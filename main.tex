...
\begin{titlepage}
    \centering
    \textbf{\large{Thompson Sampling for Bandit Convex Optimization\\(Version 4.2)}}
...
\begin{lemma}
    ...
    Let $X \geq 0$ be a random variable with $\E[X^2] \leq \infty$.
...
\begin{align*}
    \pi_t(\theta) := \exp\left(
    -\frac{1}{2c} \norm{\theta - \mu}^2_{} \sum_{s=1}^t (Y_s - f_\theta(X_s))^2
    \right)
\end{align*}
