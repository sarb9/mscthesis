...
\begin{titlepage}
    \centering
    \textbf{\large{Thompson Sampling for Bandit Convex Optimization\\(Version 4.0)}}
...
\begin{definition}\label{def:cover}
    ...
    \begin{itemize}
        \item \textit{Closure:} For all $k \in [N]$, $\conv(\sF_k) \subset \sF$.
              % \item \textit{Common near-minimizer:} $x_k$ is close to all the points in $\{\tilde{x}_f: f \in \sF_k\}$: $\sup_{f \in \sF_k} \norm{\tilde x_f - x_k} \leq \epsilon$.
...
\begin{figure}[h]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Illustration of the function $f_\theta$ for $\epsilon = 0.5$ and $\theta = 1$ in 1 dimension. It can be seen that $f_\theta$ is convex, minimized at $\theta$ and lies below $f$.}
    \label{fig:ftheta}
\end{figure}
...
\begin{definition}[Known Ridge Function Class]
    Let $\Theta \subseteq \R^d$ be a set of parameters and $\ell:\R\to[0, b]$ be a convex, non-increasing function, for some $b > 0$.
    Define $\sF_{\Theta, \ell}$ be the space of all functions $f:\cK \to [0, b]$ such that
...
Precisely, let $f_\theta(\cdot) = \ell(\ip{\theta, \cdot})$,
and, for every subset $\Theta' \subseteq \Theta$, let $\sF_{\Theta'} = \{f_\theta(\cdot) : \theta \in \Theta'\}$.
Define $\sigma \in \cP(\Theta)$ such that for every measurable subset $\Theta'\subseteq \Theta$,
\begin{align}
...
\begin{restatable}{lemma}{KnownRidgeLemma}
    ...
    It holds that $(0, d) \in \IR(\sF_{\Theta,\ell})$ whenever for every $\theta \in \Theta$, it holds that
    \begin{align*}
...
\end{theorem}
\begin{proof}
    Let $\sigma$ be the corresponding distribution over the parameter space $\Theta$, as defined in \cref{eq:sigma-xi}.
...
\begin{lemma}
    ...
    such that $g(0) = 0$.
    Then for any random variable $X$ supported on $[0,b]$ for some $b > 0$, it holds that
    \begin{align*}
        \frac{
            \E[g(X)^2]
        }{
...
\begin{lemma}
    ...
    Further, let $X$ be a random variable supported on $[0, b]$ for some $b > 0$
    and $x_1 = \essinf X$.
...
\begin{proof}
    ...
    let $\sigma$ be the corresponding distribution over the parameter space $\Theta$, as defined in \cref{eq:sigma-xi}.
    Let $(\theta, X)$ be jointly distributed such that $\theta \sim \sigma$, $X = \argmin_{x \in \cK} \langle x, \theta\rangle$.
    Then $X \sim \pi_\ts^\xi$ as $\ell$ is assumed to be monotone.
...
Note that $f \in \sF_{\pb\pl}$ and is piecewise linear and minimized at $-1$ with $f(-1) = 0$ and $f(0) = \epsilon$ and $f(1) = 1$.
...
Because the loss function changes from round to round, the action $X_t$ may not minimize $f_t$. This must be reflected in the definition of the information
