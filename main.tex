...
\begin{titlepage}
    \centering
    \textbf{\large{Thompson Sampling for Bandit Convex Optimization\\(Version 4.4)}}
...
% The function class in the following lower bound only includes two linear functions,
% which means $\ts$ as defined in \ref{item:ts-adv:bad} suffers linear regret even for linear functions
% which are a subset of convex functions.
\begin{theorem}
