...
\begin{titlepage}
    \centering
    \textbf{\large{Thompson Sampling for Bandit Convex Optimization\\(Version 4.1)}}
...
\begin{definition}[Known Ridge Function Class]
    Let $\Theta \subseteq \R^d$ be a set of parameters and $\ell:\R\to\R$ be a convex, non-increasing function, for some $b > 0$.
...
\begin{restatable}{lemma}{KnownRidgeLemma}
    ...
    For every $\xi \in \sP(\sF_{\Theta, \ell})$, it holds that
    \begin{align*}
        \Psi(\xi) < d,
    \end{align*}
    whenever for every $\theta \in \Theta$, it holds that
    \begin{align*}
